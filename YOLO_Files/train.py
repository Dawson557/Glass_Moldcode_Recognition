...
from YOLO_Files import test # Import test.py to get mAP after each epoch
...
def train(
	...
	init_seeds()
	weights = r'C:\Users\admin\Desktop\Moldcode_Network\weights' + os.sep
	...
	if resume:  # Load previously saved model
		if transfer:  # Transfer learning
			chkpt = torch.load(weights + 'converted.pt', map_location=device)
		...
		if '-tiny.cfg' in cfg:
			#cutoff = load_darknet_weights(model, weights + 'yolov3-tiny.conv.15') #only good for 3 channel
			pass
		else:
	...
	#     dist.init_process_group(backend=opt.backend, init_method=dist_url, world_size=world_size, rank=rank)
	...
	for epoch in range(start_epoch, epochs):
		...
		for i, (imgs, targets, _, _, _) in enumerate(dataloader):
			imgs = imgs.to(device)
...
if __name__ == '__main__':
	...
	parser.add_argument('--cfg', type=str, default=r'C:\Users\admin\Desktop\Moldcode_Network\YOLO_Files\bottles_yolov3-tiny.cfg', help='cfg file path')
