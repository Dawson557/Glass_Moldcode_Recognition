...
def labels_to_class_weights(labels, nc=10):
...
def labels_to_image_weights(labels, nc=10, class_weights=np.ones(10)):
...
#TODO: Finish this loss method for student YOLO network. This is not exactly straightforward and I'm out of time for this internship
# Maybe check this paper out as a starting point https://arxiv.org/pdf/1805.06361.pdf
def compute_student_loss(p, targets, model, teacher_out):  # predictions, targets, model
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor
    lxy, lwh, lcls, lconf = ft([0]), ft([0]), ft([0]), ft([0])
    txy, twh, tcls, indices = build_targets(model, targets)
    ...
    for i, pi0 in enumerate(p):  # layer i predictions, i
        ...
        if len(b):  # number of targets
            ...
            teach_pred = teacher_out[b, a, gj, gi]
...
def plot_images(imgs, targets, fname='images.jpg'):
    ...
    for i in range(bs):
        ...
        boxes[[0, 2]] *= w
        boxes[[1, 3]] *= h
        plt.subplot(ns, ns, i + 1).imshow(imgs)
